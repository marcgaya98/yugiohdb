...
async function importCardsByKonamiId() {
    console.log('[START] Importación de cartas por konami_id');
    const errores = [];
    const notFoundKonamiIds = [];
    const cardsToFetch = [];
    const localCardsByKonamiId = {};

    // 1. Prepara el mapeo por konami_id SOLO para la primera carta
    const firstCard = cardsData[0];
    if (firstCard) {
        const konamiId = String(firstCard.Code || firstCard.konami_id || firstCard.code); // Ajusta según tu JSON real
        if (konamiId) {
            localCardsByKonamiId[konamiId] = firstCard;
            console.log(`[INFO] Primera carta a procesar: ${firstCard.Card} (konami_id: ${konamiId})`);
        }
    }

    // 2. Busca en la base de datos por code (konami_id) SOLO para la primera carta
    ...
    console.log('[DONE] Importación fisssssssssssssnalizada');
}
